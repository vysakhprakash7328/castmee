        ...
        if user is not None:
            ...
            try:
                ...
                try:
                    ...
                    return Response(
                        {
                            "detail": "user is not an artist or producer ",
                            "success": False,
                        },
                        status=status.HTTP_406_NOT_ACCEPTABLE,
                    )
            if producer is not None:
                if producer.admin_approved == False:
                    return Response(
                        {
                            "detail": "Please wait untill admin approves",
                            "success": False,
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                response_data = ArtistExtendedSerializerView(
                    artist_extended, context={"artist": True}
                ).data
            ...
            return Response(
                {"data": response, "success": True}, status=status.HTTP_200_OK
            )
        else:
            return Response(
        ...
        if user_type is not None:
            if user_type == "producer":
                ...
                response_data = {
                    ...
                    "consider_me_for": [
                        {obj.id: obj.name} for obj in ConsiderMe.objects.all()
                    ],
                    "available_for": [
                        {obj.id: obj.name} for obj in AvailableFor.objects.all()
                    ],
                    "preferred_format": [
                        {obj.id: obj.name} for obj in PreferredFormat.objects.all()
                    ],
                    "preferred_genre": [
                        {obj.id: obj.name} for obj in PreferredGene.objects.all()
                    ],
...
class ArtistExtendedUpdateAPIView(APIView):
    ...
    def put(self, request):
        ...
        if "multipart/form-data" in request.content_type:
            ...
            if consider_me_for is not None:
                ...
                consider_me_for_instances = ConsiderMe.objects.filter(
                    id__in=consider_me_for
                )
            ...
            if available_for is not None:
                ...
                available_for_instances = AvailableFor.objects.filter(
                    id__in=available_for
                )
            ...
            if preferred_format is not None:
                ...
                preferred_format_instances = PreferredFormat.objects.filter(
                    id__in=preferred_format
                )
            ...
            if preferred_genre is not None:
                ...
                preferred_genre_instances = PreferredGene.objects.filter(
                    id__in=preferred_genre
                )
        ...
        serializer = ArtistExtendedSerializer(
            artist_extended, data=request.data, partial=True
        )
        if serializer.is_valid():
            serializer.save()
            data = ArtistExtendedSerializerView(
                artist_extended, context={"artist": True}
            ).data
            return Response(
                {"detail": data, "success": True}, status=status.HTTP_200_OK
            )
        return Response(
    ...
    def get(self, request):
        ...
        artist_extended = ArtistExtended.objects.get(artist_id=artist.id)
        serialized_data = ArtistExtendedSerializerView(
            artist_extended, context={"artist": True}
        ).data
        return Response(
            {"detail": serialized_data, "success": True}, status=status.HTTP_200_OK
        )
...
class ProducerExtendedAPIView(APIView):
    ...
    def put(self, request):
        ...
        serializer = ProducerExtendedSerializer(
            producer_extended, data=request.data, partial=True
        )
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"detail": serializer.data, "success": True}, status=status.HTTP_200_OK
            )
        return Response(
            {"detail": serializer.errors, "success": False},
            status=status.HTTP_400_BAD_REQUEST,
...
class FilterApi(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {"detail": "requested producer not found", "success": False},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class WishlistSaver(APIView):
    ...
    def get(self, request):
        ...
        wishlist = (
            WishList.objects.filter(producer_id=producer.id)
            .values_list("artist", flat=True)
            .distinct()
        )

        related_artists = ArtistExtended.objects.filter(artist_id__in=wishlist)
        response = ArtistExtendedSerializerView(
            related_artists, many=True, context={"producer_id": producer.id}
        ).data

        return Response(
            {"detail": response, "success": True}, status=status.HTTP_200_OK
        )
...
class RequestContactApprover(APIView):
    ...
    def get(self, request):
        ...
        request_contact = RequestContact.objects.filter(artist_id=artist.id)
    ...
    def put(self, request):
        ...
        try:
            ...
            return Response(
                {"detail": "Contact list not found", "success": False},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
def artist_dropdowns(request):
    ...
    return Response(
        {"detail": response_data, "success": True}, status=status.HTTP_200_OK
    )
...
class RequestContactSave(APIView):
    ...
    def get(self, request):
        ...
        requested_contacts = (
            RequestContact.objects.filter(producer_id=producer.id)
            .values_list("artist", flat=True)
            .distinct()
        )
...
def get_notifications(request):
    data = NotificationTracker.objects.filter(user_id=request.user.id).values_list(
        "notification", flat=True
    )
    return Response({"detail": data, "success": True}, status=status.HTTP_200_OK)
